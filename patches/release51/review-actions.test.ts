...
it('still blocks an overlong title even when its content was reviewed',()=>{
  const $=render(change(['source-reviewed-v1: Checked against the source.'],'Wild and Free Wood Print for Your Home'));
  expect($('button').filter((_,e)=>$(e).text()==='Accept and apply').is('[disabled]')).toBe(true);
  expect($.text()).toContain('five words');
});
