...
export async function generateCopy(storeId:string,p:Payload,kind:string,feature:'seo'|'description',facts:Facts) {
  ...
  const review = z.object({allowed:z.boolean(),reason:z.string()}).parse(await ask(storeId,keys,voice+copyReviewInstructions(feature),[{role:'user',content:JSON.stringify({source,before,after,feature})}],format('copy_review',{allowed:{type:'boolean'},reason:string})));
