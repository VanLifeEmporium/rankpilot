...
export function ChangeReview({d,review,busy,accepting,feedback,onAction,onReplace,onEditFacts}:{
 ...
 onAction:(intent:string,id:string)=>void;onReplace:()=>void;onEditFacts:()=>void;
}) {
 ...
 if(['seo','title'].includes(review.feature)) {
   ...
   if((!title||title.split(/\s+/u).length>5||title.length>60)&&!reviewBlockers.length)reviewBlockers.push('Generate a replacement with up to five words, max 60 characters.');
 }
 return (
          <>
            ...
            <div className="preview-meta">
              ...
              {["title","seo"].includes(review.feature) && <span>{review.feature === "seo" ? "Changes the suggested Google title, not the name shown on your product page." : "Updates the visible page title."} Up to five words, max 60 characters, including any brand suffix. Both limits apply.</span>}
            </div>
            ...
            {feedback?.ok===false && <><p role="alert">{feedback.message}</p>{feedback.comparison && <table><thead><tr><th>Search field</th><th>Accepted proposal</th><th>Saved in Shopify</th></tr></thead><tbody>{feedback.comparison.map(row=><tr key={row.field}><th>{row.field} — {row.matches?'Matches':'Differs'}</th><td>{row.accepted || 'Not set'}</td><td>{row.live || 'Not set'}</td></tr>)}</tbody></table>}</>}
              {review.status === "pending" && (
                <ReviewActions blockers={reviewBlockers} busy={busy} accepting={accepting} demo={d.demo}
                  onReject={()=>{onAction('reject',review.id);}}
