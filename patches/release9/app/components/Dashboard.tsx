...
export function Dashboard({d,issues,busy,onFix,onReview,onReviewReady}:{d:Data;issues:Issue[];busy:boolean;onFix:(issues:Issue[])=>void;onReview:(id:string)=>void;onReviewReady:()=>void}){
 ...
 <section className="card"><div className="card-head"><h2>Fix these next</h2><button className="button" onClick={onReviewReady}>Review ready fixes ({ready.length})</button></div><p className="muted">Clear failures first, then useful improvements. Nothing is published until you approve.</p>
 {(all?groups:groups.slice(0,5)).map(g=>{const ids=[...new Set(g.items.map(i=>i.resourceId))];const proposals=ready.filter(c=>ids.includes(c.resourceId)&&g.items.some(i=>i.feature===c.feature));return <div className="priority" key={g.code}><span className={'badge '+(g.priority===0?'red':g.priority<3?'amber':'neutral')}>{g.priority===0?'High':g.priority<3?'Medium':'Optional'}</span><div><h3>{g.code==='google-listings'?'Improve Google titles and summaries':findingName(g.code)}</h3><small>{g.count} affected {g.count===1?'page':'pages'}</small><details><summary>Why and how to fix</summary><p>{g.items[0].detail}</p><Link to={`/app/audit?finding=${encodeURIComponent(g.items[0].code)}`}>See affected pages and evidence</Link></details></div><button className="button" disabled={busy} onClick={()=>onFix([...g.items].sort((a,b)=>Number(proposals.some(c=>c.resourceId===b.resourceId&&c.feature===b.feature))-Number(proposals.some(c=>c.resourceId===a.resourceId&&c.feature===a.feature))))}>{proposals.length?`Review a fix (${proposals.length})`:'Inspect & fix'}</button></div>})}
 ...
 {ready.length>0 && <p><button className="text-link" onClick={()=>{setQueue(true);requestAnimationFrame(()=>document.getElementById('dashboard-review')?.scrollIntoView({block:'start'}));}}>Review as a set</button></p>}
