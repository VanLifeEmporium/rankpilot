...
export const findingNames:Record<string,string>={
 ...
 'missing-product-faq':'Answer customer questions','http-error':'Fix a page that will not load','broken-link':'Fix a broken link','missing-gtin':'Check the product barcode','large-image':'Check image loading size',
};
export function findingName(code:string){return findingNames[code] || code.replace(/[-_]/g,' ');}
