...
it('only counts actionable drafts and translates failure badges',()=>{
 ...
 expect(changeStatus('verification_failed')).toBe('Update needs a check');
});
it('protects the original brand story from supplier heuristics and product templates',()=>{
 ...
 expect(titleBrandInstructions({...defaults,titleBrandMode:'append',titleBrand:'VLE'})).toContain('There is no word limit');
});
