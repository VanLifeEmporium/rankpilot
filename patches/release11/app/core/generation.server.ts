...
async function ask(storeId:string,keys:Record<string,string>, instructions:string, input:unknown[], output:unknown) {
  ...
  let r:Response;
  try { r = await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{Authorization:`Bearer ${keys.openaiKey}`,'Content-Type':'application/json'},signal:AbortSignal.timeout(output && typeof output==='object' && 'name' in output && ['copy_proposal','copy_review'].includes(String(output.name))?20000:90000),body:JSON.stringify({model:keys.openaiModel || 'gpt-5-mini',store:false,instructions,input,text:{format:output},max_output_tokens:output && typeof output==='object' && 'name' in output && output.name==='copy_review'?1200:3000})}); } catch(e) { if(e instanceof Error && ['TimeoutError','AbortError'].includes(e.name))throw new Error('The writing service took too long. No change was made. Try preparing this preview again.');throw e; }
