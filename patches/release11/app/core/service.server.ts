...
export async function enqueueGeneration(storeId:string, ids:string[], feature:Feature, regenerate=false) {
 ...
 const fingerprint=createHash('sha256').update(JSON.stringify({version:11,feature,titleBrandMode:settings(store.settings).titleBrandMode,titleBrand:settings(store.settings).titleBrand,policy:feature==='faq'?policy:undefined,resources:resources.map(r=>[r.id,r.payload,r.facts])})).digest('hex');
...
export async function propose(
  ...
  const proposal = feature === "title" && (cleanTitle(p.title).length>65)
...
export async function proposeRedirect(
  ...
  if(client){
    ...
    if(![404,410].includes(response.status))throw new Error(`The audit recorded a missing page earlier, but a fresh check now returns HTTP ${response.status}. No redirect was created. Recheck this destination to update its findings.`);
  }
  if (!client) {
