...
export function brokenAddress(issue:Issue, domain:string, resource?:{kind:string;handle:string;payload:string}) {
 let value=issue.link?.url || (issue.code==='reported-404'?issue.resourceId.replace(/^reported:/,''):issue.code==='broken-link'?issue.detail.match(/ to (https?:\/\/\S+) returns (?:404|410)\./)?.[1]:'');
