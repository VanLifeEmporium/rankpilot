...
export async function loadUI(request: Request) {
  ...
  const [
    ...
    Promise.all(['gsc','ga4','merchant','bing','pagespeed','ga4-ai','gsc-daily','ga4-organic-daily','indexation','store-score','change-impact'].map(provider=>prisma.metric.findMany({where:{storeId:store.id,provider},orderBy:provider==='change-impact'?{createdAt:'desc'}:{period:'desc'},take:provider==='store-score'?91:provider==='change-impact'?200:12}))).then(groups=>groups.flat()),
...
export async function actionUI(request: Request) {
  ...
  try {
    ...
    switch (intent) {
      ...
      case "indexation": {
        const job=await enqueue(store.id,'indexation');
