...
export const operations = {
  ...
  products: `query Products($after:String) { products(first:30,after:$after) { pageInfo { hasNextPage endCursor } nodes { id title handle descriptionHtml updatedAt onlineStoreUrl vendor productType tags status seo { title description } seoTitle:metafield(namespace:"global",key:"title_tag") { value } seoDescription:metafield(namespace:"global",key:"description_tag") { value } collections(first:50) { nodes { title handle } } variants(first:100) { nodes { sku barcode price selectedOptions { name value } } pageInfo { hasNextPage endCursor } } media(first:100) { nodes { id alt ... on MediaImage { image { url width height } } } pageInfo { hasNextPage endCursor } } faq:metafield(key:"faqs") { jsonValue namespace } } } }`,
