...
export function useLiveWorkspace(initial:Data,section:string,actionState:string) {
 ...
 useEffect(()=>{
  ...
  const check=async()=>{
   ...
   try {
    const status=failures>=2?{revision:"fallback",workerHealthy:true}:await workspaceJSON('/app/job-status',initial.demo,controller.signal);
    ...
    if(!stopped){const active=latest.current.jobs.some(j=>['queued','running'].includes(j.status));timer=setTimeout(check,failures?Math.min(30000,2000*2**Math.min(failures,4)):active?1500:8000);}
   }
  };
