...
export async function enqueueGeneration(storeId:string, ids:string[], feature:Feature, regenerate=false) {
 ...
 const fingerprint=createHash('sha256').update(JSON.stringify({version:12,feature,titleBrandMode:settings(store.settings).titleBrandMode,titleBrand:settings(store.settings).titleBrand,policy:feature==='faq'?policy:undefined,resources:resources.map(r=>[r.id,r.payload,r.facts])})).digest('hex');
...
export async function propose(
  ...
  if(feature==='links')proposal.reasons.push(...related.map(r=>'Medium impact: '+r.reason));
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.job.create({
      data: {
        storeId,
        kind: "apply",
        payload: JSON.stringify({ changeId: id, verificationPass:7 }),
...
export async function queueSavedChecks(storeId:string) {
 ...
 for(const row of rows){
  ...
  await prisma.$transaction(async tx=>{
   ...
   await tx.job.update({where:{id:job.id},data:{status:'queued',attempts:0,runAt:new Date(),lockedAt:null,error:null,payload:JSON.stringify({...payload,verificationPass:7,verifyOnly:true})}});
  });
 }
}
export async function verifyChange(storeId:string,id:string) {
 ...
 if(!matchesField(actual,expected,change.feature)) {
  ...
  return {ok:false,message:'The live Shopify value differs from this accepted proposal. No content was changed. Compare the saved fields below before retrying.',comparison};
 }
 await prisma.$transaction([
...
export async function executeJob(job: {
  ...
  switch (job.kind) {
    ...
    case "visibility":
      await trackVisibility(job.storeId);
      return recordScore(job.storeId);
    case "recheck-link":
      return recheckLink(job.storeId,p.address,p.resourceId);
    case "indexation":
      return checkIndexation(job.storeId);
    case "pagespeed":
