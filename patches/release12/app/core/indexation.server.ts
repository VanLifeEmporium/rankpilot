...
export async function checkIndexation(storeId:string){
 ...
 const selected=eligible.slice(0,20);let failed=0;
 for(const url of selected){
  ...
  const payload=JSON.stringify({checkedAt:new Date().toISOString(),submitted:urls.size,indexed:rows.filter(r=>r.verdict==='PASS').length,inspected:rows.filter(r=>r.verdict).length,rows,errors,limited:sitemapQueue.length>0||urls.size>=5000,note:'URLs found in the current sitemap, inspected against Google’s stored index. Not a live indexability test. Checks expire after 28 days; uninspected URLs are unknown, not failures.'});
