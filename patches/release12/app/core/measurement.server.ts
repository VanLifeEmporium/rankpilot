...
export async function recordScore(storeId:string){
 const [audit,resources,observations,metrics]=await Promise.all([prisma.audit.findFirst({where:{storeId},orderBy:{createdAt:'desc'}}),prisma.resource.findMany({where:{storeId}}),prisma.observation.findMany({where:{storeId,createdAt:{gte:new Date(Date.now()-28*86400000)}}}),Promise.all(['gsc','ga4-organic-daily'].map(provider=>prisma.metric.findFirst({where:{storeId,provider},orderBy:{period:'desc'}}))).then(xs=>xs.filter(x=>x!==null))]);
 if(!audit)return;
 ...
 const value=storeScore({metrics,technical:audit.score,healthScores,observations,now:Date.now()});
