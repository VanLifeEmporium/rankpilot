...
it('queues deterministic fixes through the same review lifecycle', async () => {
 const result = await actionUI(request("title"));
 expect(("jobId" in result.data ? result.data.jobId : undefined)).toBe('generation-1');
...
it('retrieves a completed generation rather than charging again implicitly', async () => {
 ...
 const result = await actionUI(request("title"));
 expect(("message" in result.data ? result.data.message : undefined)).toContain('Existing generation result');
});
it('queues article image generation for background processing', async () => {
 mocks.rows.mockResolvedValue([{id:'resource',kind:'article'}]);
 const result = await actionUI(request('alt'));
...
it('returns an audit job identifier instead of an ambiguous recorded message', async () => {
 ...
 const result = await actionUI(req);
 expect(("jobId" in result.data ? result.data.jobId : undefined)).toBe('audit-1');
 expect(("message" in result.data ? result.data.message : undefined)).toContain('Audit queued');
...
it('fails clearly when generation credentials are absent',async()=>{
 ...
 const result=await actionUI(request('description'));
 expect(result.data.ok).toBe(false);expect(("message" in result.data ? result.data.message : undefined)).toContain('API key');
