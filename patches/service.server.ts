...
export async function applyChange(
  ...
  const next = client ? await fetchResource(client, r.remoteId, r.kind) : withField(p, change.feature, after);
  if(!sameField(fieldValue(next,change.feature),after)) {
    await prisma.change.update({where:{id},data:{status:'verification_failed',error:'Shopify did not return the accepted value. The finding remains open. Check the change before retrying.'}});
    throw new Error('Verification failed: Shopify did not return the accepted value.');
  }
