...
const env={...process.env,DEMO_MODE:'true',DATABASE_URL:'file:/tmp/rankpilot-stability142.sqlite',SHOPIFY_APP_URL:'http://localhost:3018',ENCRYPTION_KEY:'a'.repeat(64),SHOPIFY_API_KEY:'local-test-key',PORT:'3018',HOST:'127.0.0.1'};
...
let browser;
try {
 for(let i=0;i<60;i++){try{if((await fetch('http://localhost:3018/health')).ok)break;}catch{}await wait(250);if(i===59)throw new Error(logs);}
 ...
 const context=await browser.newContext();const page=await context.newPage();page.setDefaultTimeout(10000);const errors=[];page.on('console',m=>{if(m.type()==='error')console.error('BROWSER CONSOLE',m.text());});page.on('pageerror',e=>{errors.push(e.message);console.error('PAGE ERROR',e.stack);});
 ...
 await page.goto('http://localhost:3018/app');
 ...
 await page.goto('http://localhost:3018/app/content');
 ...
 await context.route('http://localhost:3018/app**',async r=>{const response=await r.fetch();const headers={...response.headers()};delete headers['content-security-policy'];delete headers['x-frame-options'];await r.fulfill({response,headers});});
 await context.route('http://localhost:3018/release14-frame',r=>r.fulfill({contentType:'text/html',body:'<html><body style="margin:0"><iframe title="Embedded app" src="/app/products" style="border:0;width:910px;max-width:100vw;height:713px"></iframe></body></html>'}));
 for(const [width,height] of [[1024,768],[1280,800],[1440,900]]){
  await page.setViewportSize({width,height});await page.goto('http://localhost:3018/release14-frame');
  ...
  for(const [section,button] of [['products','Product details'],['collections','Collection plan']]){
   await frame.goto('http://localhost:3018/app/'+section);
  ...
  await frame.goto('http://localhost:3018/app');await app.getByRole('heading',{name:'Store Score',exact:true}).click();await page.mouse.move(800,500);await page.mouse.wheel(0,900);await expect.poll(()=>frame.evaluate(()=>document.scrollingElement.scrollTop)).toBeGreaterThan(100);
 ...
 // 15-minute production-build soak; real Polaris, isolated SQLite, simulated jobs.
 // No external merchant writes or authenticated Shopify claims.
 const evidenceRows=Array.from({length:5000},(_,i)=>({url:`https://fixture.invalid/products/item-${i}`,checkedAt:new Date().toISOString(),verdict:i%3?'PASS':'FAIL',coverageState:i%3?'Submitted and indexed':'Crawled - currently not indexed'}));
 const idxPayload=JSON.stringify({checkedAt:new Date().toISOString(),indexed:evidenceRows.filter(r=>r.verdict==='PASS').length,inspected:5000,submitted:5000,rows:evidenceRows,note:'Local stress fixture'});
 await db.metric.upsert({where:{storeId_provider_period:{storeId,provider:'indexation',period:'current'}},create:{storeId,provider:'indexation',period:'current',payload:idxPayload},update:{payload:idxPayload}});
 const runningJob=await db.job.create({data:{storeId,kind:'indexation',status:'running',dedup:storeId+':soak',payload:JSON.stringify({progress:{checked:1,total:5000}})}});
 let fullRefreshes=0,statusPolls=0,evidenceRequests=0;
 page.on('request',r=>{if(r.url().includes('/app/live-state'))fullRefreshes++;if(r.url().includes('/app/job-status'))statusPolls++;if(r.url().includes('/app/index-evidence'))evidenceRequests++;});
 await page.goto('http://localhost:3018/app');
 const fullAtStart=fullRefreshes;
 const headline=page.getByText('3333 of 5000 checked URLs indexed',{exact:true});await expect(headline).toBeVisible();
 const tile=page.locator('.metric').filter({has:headline});
 await tile.getByText('How this is measured / what to do',{exact:true}).click();
 await tile.getByText('URL evidence and checks that need attention',{exact:true}).click();
 await expect(tile.getByRole('button',{name:'Next URLs',exact:true})).toBeEnabled();
 await tile.getByRole('button',{name:'Next URLs',exact:true}).click();
 await expect(tile.getByText('2 of 500',{exact:true})).toBeVisible();
 await db.job.update({where:{id:runningJob.id},data:{payload:JSON.stringify({progress:{checked:2,total:5000}})}});
 await expect(page.getByText('2 of 5000 sitemap URLs checked. You can continue using the app.',{exact:true})).toBeVisible({timeout:15000});
 expect(fullRefreshes-fullAtStart).toBe(0);
 console.log('PASS indexation progress arrives without a full workspace reload; 5000 evidence rows are fetched in pages of 10');
 const started=Date.now(),duration=Number(process.env.SOAK_MS||900000);let iteration=0,maxInteraction=0;
 while(Date.now()-started<duration){
  await db.job.update({where:{id:runningJob.id},data:{payload:JSON.stringify({progress:{checked:3+iteration,total:5000}})}});
  const route=['','/settings','/reports','/products'][iteration%4];
  const before=Date.now();await page.goto('http://localhost:3018/app'+route);
  await expect(page.locator('html')).toHaveAttribute('data-rankpilot-ready','true');
  if(route==='/products'){const search=page.getByRole('textbox',{name:'Search pages'});await search.fill('power');await expect(search).toHaveValue('power');await search.fill('');}
  else {const disclosure=page.locator('summary').first();if(await disclosure.count())await disclosure.click();}
  await page.mouse.wheel(0,500);await page.evaluate(()=>document.body.getBoundingClientRect().height);
  maxInteraction=Math.max(maxInteraction,Date.now()-before);
  if(iteration%4===0){const other=await context.newPage();await other.goto('http://localhost:3018/app/settings');await expect(other.locator('html')).toHaveAttribute('data-rankpilot-ready','true');await other.close();}
  console.log('SOAK',Math.round((Date.now()-started)/1000)+'s',route||'/dashboard','responsive');
  iteration++;await wait(15000);
 }
 await db.job.update({where:{id:runningJob.id},data:{status:'failed',error:'Simulated interrupted inspection'}});
 await page.goto('http://localhost:3018/app');await expect(page.getByRole('button',{name:'Check next 20 URLs',exact:true})).toBeEnabled();
 console.log('PASS failed inspection restores next-batch action',JSON.stringify({iterations:iteration,statusPolls,fullRefreshes,evidenceRequests,maxInteraction}));
 if(errors.length)throw new Error(errors.join('\n'));
 console.log('PASS real Polaris components loaded; no browser exceptions');
} catch(error) {console.error('SERVER LOG',logs.slice(-5000));throw error;} finally {await browser?.close();server.kill('SIGTERM');await db.$disconnect();}
