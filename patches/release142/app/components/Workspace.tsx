...
/* eslint-disable jsx-a11y/no-noninteractive-tabindex -- This scroll region needs keyboard focus for Page Down and End inside Shopify. */
import {citationMeasure} from '../core/provider-health';
import {SearchPerformance,ConnectionActivity,MerchantFeed} from './ReportPanels';
...
import {reviewItems} from '../core/dashboard';
...
import { metricPair, visibilityTrend } from "../core/analytics";
...
const nav = ["Dashboard", "Products", "Collections", "Content", "Reports", "Settings"];
const icons = ["◈", "▦", "▤", "▧", "▥", "⚙"];
...
export default function Workspace() {
  ...
  const resourceDetail=useFetcher<Data["resources"][number]&{plan:Awaited<ReturnType<typeof pageSuggestions>>}>();
  ...
  useEffect(()=>{
    ...
    if(appliedFactResult.current===result)return;
    appliedFactResult.current=result;
    let filled=0;
  ...
  },[fetcher.data,factId,resourceDetail.data]);
  ...
  const [reviewTab,setReviewTab]=useState('ready');const [observationPage,setObservationPage]=useState(0),[observationSize,setObservationSize]=useState(10);
  const review = d.changes.find((c) => c.id === reviewId);
  const factResource = resourceDetail.data?.id===factId ? resourceDetail.data : undefined;
  const detailRequested=useRef("");
  useEffect(()=>{if(factId && detailRequested.current!==factId){detailRequested.current=factId;resourceDetail.load(`/app/resource?id=${encodeURIComponent(factId)}`);}if(!factId)detailRequested.current='';},[factId,resourceDetail]);
  ...
  const sampling=citationMeasure(d.observations,metricPair(d.metrics,'ai-sampling')[0],Date.parse(d.measuredAt));
  const recentAnswers=d.observations.filter(o=>Date.parse(d.measuredAt)-new Date(o.createdAt).getTime()<=28*86400000);
  const mentionRate = sampling.score===null?null:Math.round(100*recentAnswers.filter(o=>o.mentioned).length/recentAnswers.length);
  ...
  return (
    <div className="workspace" tabIndex={0} aria-label="RankPilot workspace">
      ...
      <div className="main-wrap">
        ...
        <main>
          ...
          {section === "dashboard" && <Dashboard d={d} issues={issues} busy={busy || generationBusy} onFix={openFix} onReview={id=>{setReviewQueue(null);setFixGroup(null);setFactId("");setReviewId(id);}} onReviewReady={()=>{setFixGroup(null);setReviewQueue(pending.map(c=>c.id));setFixIndex(0);setReviewId(pending[0]?.id || "");}}/>}
          {section === "audit" && (
            <>
                ...
                {d.resources
                  ...
                  .map((r) => (
                    <div className="content-item" key={r.id}>
                      ...
                      <Button onClick={() => setFactId(r.id)}>Review suggestions &amp; links</Button>
                    </div>
                  ))}
                {!d.resources.some(
            ...
            <>
              <div className="metrics-grid">
                ...
                <Metric
                  ...
                  value={mentionRate === null ? "Not measured" : `${mentionRate}%`}
                  caption={sampling.note}
                />
                <Metric
                  label="Citation samples"
                  value={String(d.observations.filter((o) => o.cited).length)}
                  caption="A verified match means an answer includes a citation linking to your store domain. Zero means none of the recorded answers did so."
                />
                <Metric
              ...
              <div className="two-cols">
                <section className="card">
                  ...
                  <p>Installing an embed does not prove that a page passes. The verdict below comes from the last storefront scan. Ratings and GTINs are added only when genuine data exists; collection markup requires the theme extension to be published and enabled.</p>
                  <SchemaInventory rows={d.discoveries.schemas||[]}/>
                </section>
                <section className="card">
                  <SectionHeading title="AI crawler readiness"/>
                  {d.discoveries.robots?.length ? (
                    d.discoveries.robots.map((r: {agent:string;home:boolean;product:boolean;samples?:string[];directives?:string}) => (
                      ...
                      <details><summary>Tested URLs and robots rules</summary>{(r.samples||[d.domain+"/",d.domain+"/products/example"]).map(url=><p key={url}>{url}</p>)}<pre>{r.directives||"This older scan did not retain directives. Run a new store scan."}</pre><p>This samples crawler rules, not whether a bot actually fetched the page.</p></details></div>
                    ))
                  ) : (
                    <p className="muted">
              ...
              <section className="card">
                <div className="card-head">
                  ...
                  <Download demo={d.demo} className="button" url="/app/export?type=llms">Download llms.txt draft</Download>
                </div>
                <p>
              ...
              <section className="card">
                <div className="card-head">
                  <div>
                    ...
                    <p className="muted">
                      Each run samples two buyer questions per connected
                      provider, rotating through the question set. Provider API usage may be billed. Failed requests never count as a zero result.
                    </p>
                  </div>
                  <Button
                    primary
                ...
                <label>Answers per page<select value={observationSize} onChange={e=>{setObservationSize(Number(e.target.value));setObservationPage(0);}}>{[10,25,50].map(n=><option key={n}>{n}</option>)}</select></label><div><button className="button" disabled={!observationPage} onClick={()=>setObservationPage(observationPage-1)}>Previous answers</button> {observationPage+1} of {Math.max(1,Math.ceil(d.observations.length/observationSize))} <button className="button" disabled={(observationPage+1)*observationSize>=d.observations.length} onClick={()=>setObservationPage(observationPage+1)}>Next answers</button></div>
                {d.observations.length ? (
                  <div className="table-wrap">
                    <table>
                      ...
                      <tbody>
                        {d.observations.slice(observationPage*observationSize,(observationPage+1)*observationSize).map((o) => (
                          <tr key={o.id}>
                            <td>
                              <strong>
          ...
          {section === "reports" && (
            <>
                ...
                <section className="card">
                  ...
                  {d.reports.map((r) => (
                    <div className="mini-stat" key={r.id}>
                      ...
                      <Download demo={d.demo} className="text-link" url={`/app/export?id=${r.id}`}>Download report ↓</Download>
                    </div>
                  ))}
                  {!d.reports.length && (
                    <p className="muted">
              ...
              <ObservedWins d={d}/><SearchPerformance metrics={d.metrics}/><section className="card">
              ...
              <ConnectionActivity d={d}/><MerchantFeed d={d} onDetails={setFactId}/>
              <PageSpeedResults domain={d.domain} metrics={d.metrics} jobs={d.jobs} demo={d.demo}/>
              <section className="card">
                <div className="card-head">
                  <SectionHeading title="Version history"/>
                  <Download demo={d.demo} className="text-link" url="/app/export?type=changes">Export full history ↓</Download>
                </div>
                <HistoryActions d={d} onReview={setReviewId}/>{historyPagination()}
              </section>
            </>
          )}
          {section === "settings" && (
      ...
      <FixDialog savedRevision={String(review?.updatedAt||fetcher.data?.message||"")} open={!!reviewId || !!fixGroup || reviewQueue!==null} title={reviewResource?.title || currentFix?.title || "Review a fix"}
        ...
        {fixGroup && <div className="fix-navigation"><label>Affected page
          ...
          </select></label><span>{fixIndex+1} of {fixGroup.length} fixes across {new Set(fixGroup.map(i=>i.resourceId)).size} pages · {remainingFixPages} still have this type of finding in the latest check</span></div>}
        {review ? <ChangeReview key={review.id} d={d} review={review} busy={busy || generation.state!=="idle"} accepting={fetcher.formData?.get('intent')==='approve'} feedback={actionTarget===review.id?fetcher.data:undefined}
          onAction={(intent,id,reason)=>submit(intent,{id,...(reason?{reason}:{})})}
      ...
      >
        ...
        {factId && !factResource && <p role="status">Loading your page and its source suggestions…</p>}
        {factResource && (
          <fetcher.Form
            method="post"
            key={factResource.id}
            ref={factsForm}
            onInput={()=>{if(factsForm.current)factsForm.current.dataset.dirty="true";}}
            preventScrollReset
          >
            ...
            {factResource.kind === "product" && (
              <>
                ...
                <p role="status" className="fact-feedback">{busy ? "Reading existing details…" : factMessage || "Extracted details below are suggestions. Zero confirmed means you have not checked them yet, not that extraction failed."}</p>
                ...
                {Object.entries(factLabels).map(([k, label]) => {
                  const saved:Facts=JSON.parse(factResource.facts);
                  const f:Facts=Object.fromEntries(Object.entries({...factResource.plan.suggestions.facts,...saved}).map(([name,value])=>[name,value.value?.trim()?value:factResource.plan.suggestions.facts[name]||value]));
                  return (
                    <div className="fact-field" key={k}><details><summary>Why add {label.toLowerCase()}?</summary><p>This gives customers a specific answer about the product. Copy an accurate value from the description or supplier document, add its source and confirm only after checking. Leave it blank when unknown.</p></details>
                      <small>{f[k]?.confirmed?"Confirmed against your reference":f[k]?.value?"Suggested from existing information · check accuracy":"Unknown — add a checked source or leave blank"}</small>
                      <label>
                        {label}
