...
export type RemoteNode={id:string;title:string;handle:string;descriptionHtml?:string;body?:string;seo?:{title?:string|null;description?:string|null};seoTitle?:{value:string};seoDescription?:{value:string};media?:{nodes:RemoteMedia[];pageInfo:PageInfo};image?:RemoteImage;collections?:{nodes:{title:string;handle?:string}[]};vendor?:string;productType?:string;variants?:{nodes:{sku:string;barcode:string;price:string}[];pageInfo:PageInfo};faq?:{namespace:string;jsonValue:Payload['faqs']};onlineStoreUrl?:string;blog?:{id:string;handle:string};isPublished?:boolean;status?:string;updatedAt?:string};
export type GraphQL = (
  query: string,
...
export const operations = {
  ...
  products: `query Products($after:String) { products(first:30,after:$after) { pageInfo { hasNextPage endCursor } nodes { id title handle descriptionHtml updatedAt onlineStoreUrl vendor productType status seo { title description } collections(first:50) { nodes { title handle } } variants(first:100) { nodes { sku barcode price } pageInfo { hasNextPage endCursor } } media(first:100) { nodes { id alt ... on MediaImage { image { url width height } } } pageInfo { hasNextPage endCursor } } faq:metafield(key:"faqs") { jsonValue namespace } } } }`,
  collections: `query Collections($after:String) { collections(first:50,after:$after) { pageInfo { hasNextPage endCursor } nodes { id title handle descriptionHtml updatedAt seo { title description } image { id url altText width height } } } }`,
...
export function normalise(node: RemoteNode, kind: string): Payload {
  ...
  return {
    ...
    seo: node.seo
      ...
      : {
          title: node.seoTitle?.value || "",
          description: node.seoDescription?.value || "",
        },
