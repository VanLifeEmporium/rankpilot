...
it('uses readable values and safely escapes unexpected preview content',()=>{
 ...
 expect(html).toContain('Search title');expect(html).not.toContain('<script>');expect(html).not.toContain('"title":');
...
it('checks nested offers without labelling distinct anonymous offers duplicates',()=>{
 const {nodes}=schemaNodes('<script type="application/ld+json">'+JSON.stringify({'@type':'Product',name:'Mug',offers:[{'@type':'Offer',price:'2',priceCurrency:'GBP',availability:'InStock'},{'@type':'Offer',price:'3'}]})+'</script>');
