...
export function inspectPage(html:string,url:string,resourceId:string,title:string){
 ...
 if(!pageTitle)add('live-title-missing',`No title in the HTML served at ${url}.`);else if(pageTitle.length>60)add('live-title-long',`The theme serves a ${pageTitle.length}-character title at ${url}. Check the actual search preview; display width varies.`,'notice');
