...
export async function proposeRedirect(
  ...
  const allowed=(value:string)=>/^\/(?:[a-zA-Z0-9][a-zA-Z0-9_./-]*)$/.test(value)&&!value.includes('..')&&!/^\/(cart|checkout|account|admin|apps)(\/|$)/i.test(value);
  if(!allowed(path)||!allowed(target)||path===target)throw new Error('Choose distinct store paths, excluding cart, checkout and account actions.');
  const client = await clientFor(storeId);
  if(client){
    const store=await prisma.store.findUniqueOrThrow({where:{id:storeId}});
  ...
  if (!client) {
    ...
    if(!records.some(r=>'/collections/'+r.handle===target)) throw new Error('Choose a known collection destination in demo mode.');
  }
