...
export function actionGroups(issues:Issue[]) {
 ...
 for(const i of issues){const key=['missing-meta-title','missing-meta-description','duplicate-title','duplicate-meta-title','duplicate-meta-description','long-meta-title','long-title','long-meta-description'].includes(i.code)?'google-listings':i.code;groups.set(key,[...(groups.get(key)||[]),i]);}
 return [...groups].map(([code,items])=>({code,items,count:new Set(items.map(i=>i.resourceId)).size,priority:Math.min(...items.map(issuePriority))})).sort((a,b)=>a.priority-b.priority||b.count-a.count);
}
