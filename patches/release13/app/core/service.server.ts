...
import {relevantLinks} from './link-intelligence';
...
export async function enqueueGeneration(storeId:string, ids:string[], feature:Feature, regenerate=false) {
 ...
 const fingerprint=createHash('sha256').update(JSON.stringify({version:13,feature,titleBrandMode:settings(store.settings).titleBrandMode,titleBrand:settings(store.settings).titleBrand,policy:feature==='faq'?policy:undefined,resources:resources.map(r=>[r.id,r.payload,r.facts])})).digest('hex');
...
export async function propose(
  ...
  const feedback=await prisma.event.findMany({where:{storeId,message:'Change rejected'},orderBy:{createdAt:'desc'},take:30});
  ...
  if (pending) {
    ...
    const outdated = ((feature === "seo" || feature === "title") && ((t:string)=>!t || t.length>255)(String(feature === "seo" ? JSON.parse(pending.after).title : JSON.parse(pending.after)).trim())) || (["seo","description"].includes(feature) && !reasons.some(trustedSourceReview)) || (feature === "alt" && !reasons.some(r=>r.startsWith("image-reviewed-v1:")));
  ...
  const related=relevantLinks(resource,linkTarget?relatedResources.filter(r=>r.id===linkTarget):relatedResources,search?JSON.parse(search.payload).rows:undefined,!!linkTarget);
...
export async function verifyChange(storeId:string,id:string) {
 ...
 if(!matchesField(actual,expected,change.feature)) {
  const comparison=change.feature==='seo' ? (['title','description'] as const).map(field=>({field,accepted:String(expected?.[field] ?? ''),live:live.seo[field],matches:matchesField({title:live.seo[field],description:''},{title:expected?.[field],description:''},'seo')})) : undefined;
...
export async function executeJob(job: {
  ...
  switch (job.kind) {
    ...
    case "indexation":
      {const result=await checkIndexation(job.storeId);await recordScore(job.storeId);return result;}
    case "pagespeed":
...
export async function proposeLinkRemoval(storeId:string,resourceId:string,address:string,target?:string){
 ...
 if(target){if(!validRedirectPath(target,true))throw new Error("Choose a valid store destination.");if(client)await verifyRedirectTarget(storeId,target);}
