...
export function technicalHealth(input:ScoreInput){
 ...
 const schema=schemas.length?100*schemas.filter(s=>s.types.length&&s.valid).length/schemas.length:null;
 const recent=(date:string|undefined)=>!!date&&input.now-Date.parse(date)<=28*86400000&&input.now>=Date.parse(date);
 const parts=[{label:'Catalogue setup',weight:70,value:input.technical??null},
 ...
 {label:'Scanned structured data',weight:10,value:schema}];
...
export function storeScore(input:ScoreInput){
 ...
 {key:'technical',label:'Technical health',weight:20,value:technical.score,low:technical.partial,note:'Catalogue setup 70%, mobile speed 10%, inspected index coverage 10%, scanned structured data 10%. Unmeasured parts are excluded and weights rescaled; samples are not whole-store coverage.'},
 ...
 return {version:2,technical,score:raw===null?null:capped?Math.min(55,raw):raw,parts,coverage,capped,limited:parts.some(p=>p.low)||coverage<100,search,readiness,ai,cited,samples:samples.length};
}
