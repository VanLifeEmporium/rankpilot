...
it('uses actual blog searches and preserves a manually saved phrase',async()=>{
 ...
 const plan=await pageSuggestions(storeId,r.id);expect(plan.suggestions.primary).toBe('winter campsite prices');expect(plan.keywordSource).toContain('Actual Search Console');expect(plan.savedKeyword).toBe('My chosen phrase');
