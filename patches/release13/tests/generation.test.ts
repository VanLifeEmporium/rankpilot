...
it('accepts a six-word SEO title without a word-count gate',()=>{
 expect(()=>validateCopy('Capacity 350 ml.',{}, {...proposal,title:'One Two Three Four Five Six'},'seo')).not.toThrow();
});
...
it('returns a sourced reviewable draft when the writing provider times out',async()=>{
 vi.stubGlobal('fetch',vi.fn().mockRejectedValue(new DOMException('Timed out','TimeoutError')));
 const result=await generateCopy('s',p,'product','seo',{});expect(result.after).toEqual({title:p.title,description:'A compact camping mug. Capacity 350 ml.'});expect(result.reasons.join(' ')).toContain('source-extract-v1:');expect(result.reasons.join(' ')).not.toContain('source-reviewed-v1:');
});
it('generates and reviews a sourced title over five words and over sixty characters',async()=>{
 const title='10 Best Scenic Campervan Stays in Southern England | Views Worth the Drive';
 const output={...proposal,title,description:'Read the guide to scenic campervan stays in Southern England.',evidence:[{claim:title,quote:title}]};
 const f=vi.fn().mockResolvedValueOnce(answer(output)).mockResolvedValueOnce(answer({allowed:true,reason:'An accurate editorial identity.'}));vi.stubGlobal('fetch',f);
 const result=await generateCopy('s',{...p,title,descriptionHtml:title},'article','seo',{});
 expect(result.after).toEqual({title,description:output.description});expect(result.reasons.join(' ')).toContain('Google may shorten');
});

it('lets a valid writing response complete after the former four-second deadline',async()=>{
 vi.useFakeTimers();const timeout=vi.spyOn(AbortSignal,'timeout');
 try {
 const f=vi.fn().mockImplementationOnce(()=>new Promise(resolve=>setTimeout(()=>resolve(answer(proposal)),5000))).mockResolvedValueOnce(answer({allowed:true,reason:'Accurate sourced facts retained.'}));vi.stubGlobal('fetch',f);
 const request=generateCopy('s',p,'product','seo',{});await vi.advanceTimersByTimeAsync(5001);const result=await request;
 expect(result.reasons.join(' ')).toContain('source-reviewed-v1:');expect(f).toHaveBeenCalledTimes(2);expect(timeout).toHaveBeenCalledWith(45000);
 }finally{timeout.mockRestore();vi.useRealTimers();}
});
it('retries a transient writing response once before a sourced fallback',async()=>{
 const f=vi.fn().mockResolvedValueOnce(new Response('{}',{status:503})).mockResolvedValueOnce(answer(proposal)).mockResolvedValueOnce(answer({allowed:true,reason:'Accurate sourced facts retained.'}));vi.stubGlobal('fetch',f);
 expect((await generateCopy('s',p,'product','seo',{})).reasons.join(' ')).toContain('source-reviewed-v1:');expect(f).toHaveBeenCalledTimes(3);
});
