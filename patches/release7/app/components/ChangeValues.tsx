...
export function ChangeValues({value,feature}:{value:unknown;feature:string}){
 ...
 if(['seo','redirect'].includes(feature))return <dl className="review-values">{(feature==='seo'?['title','description']:['path','target']).map(key=><div key={key}><dt>{({title:'Google title',description:'Google summary',path:'Old address',target:'New destination'} as Record<string,string>)[key]}</dt><dd>{readable(fields[key])||'Not set'}</dd></div>)}</dl>;
