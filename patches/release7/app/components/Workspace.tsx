...
import {changeStatus,percentage,reviewable} from '../core/workflow-ui';
...
const icons = ["◈", "⊞", "✓", "▣", "▦", "▤", "✳", "▥", "⚙"];
const sectionNames:Record<string,string>={Dashboard:'Overview',Audit:'Find issues',Reviews:'Review & apply',Products:'Products',Collections:'Collections',Content:'Blog drafts',AEO:'Help AI answer questions',Reports:'Results & history',Settings:'Settings'};
...
export default function Workspace() {
  ...
  const {data:d,refreshError,workerHealthy}=useLiveWorkspace(initial,section,fetcher.state);
  ...
  const firstSteps=sortedIssues.filter(i=>issuePriority(i)<9 && i.feature).slice(0,5);
  ...
  const reviewJob=review ? generating.find(j=>{try{const p=JSON.parse(j.payload);return p.feature===review.feature && Array.isArray(p.ids) && p.ids.includes(review.resourceId);}catch{return false;}}):undefined;
                  ...
                  <tr key={r.id}>
                    ...
                    <td>
                      <Button onClick={() => setFactId(r.id)}>
                        {r.kind === "product" ? "Product details" : "Keyword"}
                      </Button>
                    </td>
                  </tr>
                );
    ...
    <div className="workspace">
      ...
      <div className="main-wrap">
        ...
        <main>
          ...
          <p className="notice">You are in control: nothing changes until you approve it. You can restore the previous version of saved page edits from Results &amp; history.</p>
          ...
          {section === "reviews" && pending.length>0 && <BatchReview changes={pending} resources={d.resources}/>}
          ...
          {section === "dashboard" && (
            <>
              <div className="metrics-grid">
                <Metric
                  label="How easy your shop is to find"
                  value={audit ? `${audit.score}/100` : "—"}
                  caption={audit ? `${audit.score>=80?"Your store passed most checks":"There are useful gaps to fill"}. ${new Set(issues.filter(i=>issuePriority(i)<9).map(i=>i.resourceId)).size} pages have suggested improvements. This is not a Google ranking.` : "Run a store check to find useful improvements"}
                  accent
                />
                <Metric
                  label="Confirmed product details"
                  value={percentage(audit?.aeoScore)}
                  caption="Checked details help us write accurate answers for shoppers and AI assistants"
                />
                <Metric
                  label="Visits from search engines"
                  value={
                    organic === undefined
                      ? "—"
              ...
              {firstSteps.length>0 && <section className="card"><h2>Start with these {firstSteps.length} improvements</h2><p>These fill clear gaps first. This is a practical order, not a promise of higher rankings.</p><ol>{firstSteps.map(i=><li key={findingKey(i)}><strong>{i.title}</strong>: {findingName(i.code)}. <Link to={`/app/audit?finding=${encodeURIComponent(i.code)}`}>See suggestion</Link></li>)}</ol></section>}
              <div className="dashboard-grid">
                <section className="card priority-card">
                  ...
                  {[
                    {
                      title: "Fill missing Google titles and summaries",
                      body: "Help shoppers understand what each page offers before they visit.",
                      count: new Set(
                        issues
                          .filter(
                            (i) =>
                              ["missing-meta-title", "missing-meta-description"].includes(
                                i.code,
                    ...
                    <div className="priority" key={item.icon}>
                      ...
                      <button
                        ...
                        onClick={() =>
                          optimise(
                            [...new Set(issues.filter(i=> item.feature==='alt' ? i.code==='missing-alt' : item.feature==='faq' ? i.code==='missing-product-faq' : ['missing-meta-title','missing-meta-description'].includes(i.code)).map(i=>i.resourceId))],
                            item.feature,
                          )
                        }
                      >
                ...
                <section className="card progress-card">
                  ...
                  <p>
                    Start with one suggestion or review a small set together. You can restore the previous wording after saving.
                  </p>
                  <Button
                    primary
          ...
          {section === "settings" && (
            <>
              <fetcher.Form method="post">
                <input type="hidden" name="intent" value="settings" /><section className="card"><SectionHeading title="Search title preferences"/><p>Every suggested search title must fit five words and 60 characters in total.</p><details><summary>Why this matters and how to use it</summary><p>Keep a useful product identity first. Choose whether to retain, omit or add your store name. The same total limits apply to the suffix; a long brand may leave too little room for a useful title. These preferences affect new previews and never publish changes.</p></details><label>Store brand<input name="titleBrand" defaultValue={d.settings.titleBrand}/></label><label>Brand suffix<select name="titleBrandMode" defaultValue={d.settings.titleBrandMode || 'preserve'}><option value="preserve">Keep an existing suffix when it fits</option><option value="omit">Omit the store suffix</option><option value="append">Add the store brand</option></select></label></section>
                <section className="card">
        ...
        {review && (
          <>
            ...
            <div className="preview-meta">
              ...
              {["title","seo"].includes(review.feature) && <span>{review.feature === "seo" ? "Changes the suggested Google title, not the name shown on your product page." : "Updates the visible page title."} Maximum five words and 60 characters, including any brand suffix.</span>}
            </div>
            ...
            <details><summary>Why we suggested this change</summary><ul className="reasons">
            ...
            {review.approvedBy && !['draft','redirect'].includes(review.feature) && ['verification_failed','conflict'].includes(review.status) && <Button disabled={busy} onClick={()=>submit('verify',{id:review.id})}>Check saved result</Button>}
            ...
            {fetcher.data?.ok===false && <><p role="alert">{fetcher.data.message}</p>{fetcher.data.comparison && <table><thead><tr><th>Search field</th><th>Accepted proposal</th><th>Saved in Shopify</th></tr></thead><tbody>{fetcher.data.comparison.map(row=><tr key={row.field}><th>{row.field} — {row.matches?'Matches':'Differs'}</th><td>{row.accepted || 'Not set'}</td><td>{row.live || 'Not set'}</td></tr>)}</tbody></table>}</>}
              ...
              }).slice(0,1).map(j=><Button key={j.id} disabled={busy} onClick={()=>submit('retry',{id:j.id})}>Try saving again</Button>)}
