...
export function auditCatalogue(
  ...
  for (const r of resources) {
    ...
    if (
      /wholesale|dropship|hot sale|best quality|100% brand new/i.test(
        p.descriptionHtml,
      )
    )
      add(
        "supplier-language",
        "warning",
        "Supplier-style phrases detected. This is a heuristic, not proof of plagiarism.",
        "description",
      );
