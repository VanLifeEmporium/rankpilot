import {collectionCopy,pageSuggestions} from '../app/core/suggestions.server';
import {beforeEach,afterAll,it,expect,vi} from 'vitest';
const remote=vi.hoisted(()=>({node:{} as RemoteNode,reads:0,writes:0,staleReads:0,previousSeo:null as RemoteNode["seo"]|null,delay:0,discard:false,reject:false,editorFormat:false}));
vi.mock('../app/shopify.server',()=>({unauthenticated:{admin:async()=>({admin:{graphql:async(query:string,options:{variables?:{input?:Partial<RemoteNode> & {image?:{altText?:string}},metafields?:{key:string;value:string}[],files?:{id:string;alt:string}[]}})=>{
 ...
 if(!remote.discard){
  ...
  if(remote.editorFormat && remote.node.descriptionHtml)remote.node.descriptionHtml=remote.node.descriptionHtml.replace(/<\/p>/g,"</p>\n").replace(/&amp;/g,"&#38;").replace(/<b>/g,"<strong>").replace(/<\/b>/g,"</strong>");
...
import {audit,propose,approve,applyChange,verifyChange,queueSavedChecks,enqueueGeneration,executeJob,tick} from '../app/core/service.server';
import {generateCopy} from '../app/core/generation.server';
const storeId='workflow-integration';
beforeEach(async()=>{
 for(const model of ['resource','audit','change','job','event','metric'] as const)await (prisma[model] as unknown as {deleteMany(args:{where:{storeId:string|{in:string[]}}}):Promise<unknown>}).deleteMany({where:{storeId}});
 ...
 remote.reads=0;remote.writes=0;remote.staleReads=0;remote.previousSeo=null;remote.delay=0;remote.discard=false;remote.reject=false;remote.editorFormat=false;vi.mocked(generateCopy).mockClear();
});
afterAll(async()=>{for(const model of ['resource','audit','change','job','event','metric'] as const)await (prisma[model] as unknown as {deleteMany(args:{where:{storeId:string|{in:string[]}}}):Promise<unknown>}).deleteMany({where:{storeId}});await prisma.store.deleteMany({where:{id:storeId}});await prisma.$disconnect();});
...
it('keeps the finding open when Shopify accepts the mutation but returns the old value',async()=>{
 ...
 await expect(applyChange(storeId,c!.id)).rejects.toThrow('Confirmation pending');
 expect((await prisma.change.findUniqueOrThrow({where:{id:c!.id}})).status).toBe('verifying');
 expect(JSON.parse((await prisma.audit.findFirstOrThrow({where:{storeId}})).issues).some((i:{code:string})=>i.code==='missing-meta-title')).toBe(true);
 remote.discard=false;remote.node.seo=JSON.parse(c!.after);await verifyChange(storeId,c!.id);expect((await prisma.change.findUniqueOrThrow({where:{id:c!.id}})).status).toBe('applied');
});
...
it('applies and verifies an accepted item within five seconds while generation is blocked',async()=>{
 ...
 await enqueueGeneration(storeId,[r.id],'description');const background=tick({lane:'generation'});await started;
 try{
...
it('replaces a blocked legacy draft with a reviewed proposal that can be accepted and applied',async()=>{
 ...
 const job=await enqueueGeneration(storeId,[r.id],'seo',true);await tick({lane:'generation'});
...
it('recovers a saved product from the stored listing fields even when the SEO presentation differs',async()=>{
 const r=await resource();await audit(storeId);const c=await propose(storeId,r.id,'seo');await approve(storeId,c!.id,'owner');
 const accepted=JSON.parse(c!.after);
 remote.node.seo={title:'Presentation fallback',description:'Presentation fallback summary'};
 remote.node.seoTitle={value:accepted.title};remote.node.seoDescription={value:accepted.description};
 await prisma.change.update({where:{id:c!.id},data:{status:'verification_failed'}});
 const job=await prisma.job.findUniqueOrThrow({where:{dedup:`${storeId}:apply:${c!.id}`}});
 await executeJob(job);
 expect(remote.writes).toBe(0);
 expect((await prisma.change.findUniqueOrThrow({where:{id:c!.id}})).status).toBe('applied');
 expect(await prisma.change.count({where:{storeId,status:'applied'}})).toBe(1);
});
it('background confirmation retries never rewrite an acknowledged product update',async()=>{
 const r=await resource();const c=await propose(storeId,r.id,'seo');await approve(storeId,c!.id,'owner');remote.discard=true;
 const job=await prisma.job.findUniqueOrThrow({where:{dedup:`${storeId}:apply:${c!.id}`}});
 await tick({jobId:job.id});expect(remote.writes).toBe(1);
 expect((await prisma.change.findUniqueOrThrow({where:{id:c!.id}})).status).toBe('verifying');
 await prisma.job.update({where:{id:job.id},data:{runAt:new Date(0)}});
 await tick({jobId:job.id});expect(remote.writes).toBe(1);
 remote.node.seo=JSON.parse(c!.after);
 await prisma.job.update({where:{id:job.id},data:{runAt:new Date(0)}});
 await tick({jobId:job.id});expect(remote.writes).toBe(1);
...
it('batch approval rejects a preview changed since it was displayed',async()=>{
 const r=await resource();const c=await propose(storeId,r.id,'seo');
 await prisma.change.update({where:{id:c!.id},data:{updatedAt:new Date(c!.updatedAt.getTime()+1000)}});
 await expect(approve(storeId,c!.id,'owner',c!.updatedAt.toISOString())).rejects.toThrow('preview changed');
 expect(await prisma.job.count({where:{storeId,kind:'apply'}})).toBe(0);
});

it('stops automatic confirmation after bounded read-only retries and recovers old failures once',async()=>{
 const r=await resource();const c=await propose(storeId,r.id,'seo');await approve(storeId,c!.id,'owner');remote.discard=true;
 const job=await prisma.job.findUniqueOrThrow({where:{dedup:`${storeId}:apply:${c!.id}`}});
 for(let i=0;i<4;i++){await prisma.job.update({where:{id:job.id},data:{runAt:new Date(0)}});await tick({jobId:job.id});}
 expect(remote.writes).toBe(1);
 ...
 // Simulate the pre-release job payload for the one-time recovery pass.
 await prisma.job.update({where:{id:job.id},data:{payload:JSON.stringify({changeId:c!.id})}});
 await queueSavedChecks(storeId);
 expect((await prisma.change.findUniqueOrThrow({where:{id:c!.id}})).status).toBe('verifying');
 remote.node.seo=JSON.parse(c!.after);await tick({jobId:job.id});expect(remote.writes).toBe(1);
 expect((await prisma.change.findUniqueOrThrow({where:{id:c!.id}})).status).toBe('applied');
});

it('keeps title generation moving while older scans remain in the background lane',async()=>{
 const r=await resource();
 const scan=await prisma.job.create({data:{storeId,kind:'audit',dedup:'generation-lane-test',createdAt:new Date(0)}});
 const job=await enqueueGeneration(storeId,[r.id],'seo');
 await tick({lane:'generation'});
 expect((await prisma.job.findUniqueOrThrow({where:{id:job.id}})).status).toBe('completed');
 expect((await prisma.job.findUniqueOrThrow({where:{id:scan.id}})).status).toBe('queued');
 expect(generateCopy).toHaveBeenCalledTimes(1);
});

it('builds a relevant Off Grid collection guide, verifies editor-normalised HTML and undoes it',async()=>{
 remote.node.title='Off Grid';remote.node.handle='off-grid';remote.node.descriptionHtml='<p>Power &amp; equipment for camping.</p>';
 const r=await resource('collection');await prisma.resource.update({where:{id:r.id},data:{keyword:'off grid camping gear'}});
 for(const title of ['Solar Power Station','Portable Solar Panel','Steering Wheel Lock','Lightweight Snow Shovel'])await prisma.resource.create({data:{storeId,remoteId:title,kind:'product',title,handle:title.toLowerCase().replaceAll(' ','-'),payload:JSON.stringify({title,descriptionHtml:'',images:[],seo:{title:'',description:''},collections:['Off Grid']})}});
 const c=await collectionCopy(storeId,r.id);expect(c.after).toContain('Solar Power Station');expect(c.after).not.toContain('Snow Shovel');expect(c.after).not.toContain('Steering Wheel');
 expect(c.after).toContain('off grid');await approve(storeId,c.id,'test-owner');remote.editorFormat=true;
 await applyChange(storeId,c.id);expect((await prisma.change.findUniqueOrThrow({where:{id:c.id}})).status).toBe('applied');expect(remote.writes).toBe(1);
 await verifyChange(storeId,c.id);expect(remote.writes).toBe(1);
 await expect(collectionCopy(storeId,r.id)).rejects.toThrow('already present');
 await applyChange(storeId,c.id,true);expect((await prisma.change.findUniqueOrThrow({where:{id:c.id}})).status).toBe('rolled_back');expect(remote.writes).toBe(2);
});
it('uses actual blog searches and preserves a manually saved phrase',async()=>{
 const r=await resource('article');const p=JSON.parse(r.payload);p.blogHandle='guides';await prisma.resource.update({where:{id:r.id},data:{payload:JSON.stringify(p),keyword:'My chosen phrase'}});
 await prisma.metric.create({data:{storeId,provider:'gsc',period:'2099-01',payload:JSON.stringify({rows:[{keys:['https://example.com/blogs/guides/'+r.handle,'winter campsite prices'],impressions:20,clicks:2,position:4},{keys:['https://example.com/blogs/other/x','unrelated'],impressions:200,clicks:5,position:3}]})}});
 const plan=await pageSuggestions(storeId,r.id);expect(plan.suggestions.primary).toBe('winter campsite prices');expect(plan.keywordSource).toContain('Search Console:');expect(plan.savedKeyword).toBe('My chosen phrase');
 await prisma.metric.deleteMany({where:{storeId,provider:'gsc',period:'2099-01'}});
});
it('re-reads conflicted body content without a write and returns all comparison values',async()=>{
 const r=await resource();const p=JSON.parse(r.payload);const c=await prisma.change.create({data:{storeId,resourceId:r.id,feature:'description',before:JSON.stringify(p.descriptionHtml),after:JSON.stringify('<p>Accepted proposal.</p>'),status:'conflict',approvedBy:'owner'}});
 remote.node.descriptionHtml='<p>Newer merchant wording.</p>';
 const result=await verifyChange(storeId,c.id);expect(result.ok).toBe(false);expect(result.comparison).toEqual([{field:'description',accepted:'<p>Accepted proposal.</p>',live:'<p>Newer merchant wording.</p>',matches:false}]);
 expect(remote.writes).toBe(0);expect(JSON.parse((await prisma.resource.findUniqueOrThrow({where:{id:r.id}})).payload).descriptionHtml).toBe(remote.node.descriptionHtml);
});
it('honours a keep-current rejection even after more than 30 unrelated rejections',async()=>{
 const r=await resource();const p=JSON.parse(r.payload);const c=await prisma.change.create({data:{storeId,resourceId:r.id,feature:'seo',before:JSON.stringify(p.seo),after:JSON.stringify(p.seo),status:'rejected'}});
 await prisma.event.create({data:{storeId,message:'Change rejected',detail:JSON.stringify({resourceId:r.id,changeId:c.id,reason:'Current content is better'}),createdAt:new Date('2020-01-01')}});
 await prisma.event.createMany({data:Array.from({length:40},(_,i)=>({storeId,message:'Change rejected',detail:JSON.stringify({resourceId:'other-'+i,changeId:'other',reason:'Not relevant'})}))});
 const outcome:{message?:string}={};expect(await propose(storeId,r.id,'seo',outcome)).toBeNull();expect(outcome.message).toContain('suppressed');expect(generateCopy).not.toHaveBeenCalled();
});
