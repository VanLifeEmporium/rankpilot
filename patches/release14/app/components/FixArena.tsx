...
export function FixDialog({open,title,onClose,children,savedRevision}:{open:boolean;title:string;savedRevision?:string;onClose:()=>void;children:React.ReactNode}) {
 const dialog=useRef<HTMLDialogElement>(null);const dirty=useRef(false);const opener=useRef<HTMLElement|null>(null);
 useEffect(()=>{dirty.current=false;},[savedRevision]);
 const close=()=>{if(dirty.current&&!window.confirm('Discard your edits?'))return;dirty.current=false;dialog.current?.close();onClose();opener.current?.focus();};
 useEffect(()=>{if(open){opener.current=document.activeElement as HTMLElement;dirty.current=false;}},[open]);
 ...
 return <dialog ref={dialog} className="review-dialog change-preview" aria-labelledby="fix-title" onClose={onClose} onInput={e=>{const target=e.target as HTMLElement;if(target.tagName==="TEXTAREA"||target.tagName==="INPUT")dirty.current=true;}} onCancel={e=>{e.preventDefault();close();}}>
  <div className="dialog-head"><div><span className="eyebrow">REVIEW A FIX</span><h2 id="fix-title">{title}</h2></div><button type="button" className="close" aria-label="Close preview" onClick={close}>×</button></div>
  {children}
 </dialog>;
}
export function FindingFix({issue,relatedIssues=[],d,busy,message,feedback,onGenerate,onFacts,onReview,onAction}:{
 ...
 return <>
  <div className="review-body finding-review">
   ...
   {action.kind==='facts' && <><h3>Confirm the information first</h3><p>{diagnosis.external?'Replace the external address in your own content, remove its link while retaining the text, or mark the finding as expected. Shopify redirects cannot repair another website.':action.detail}</p><p>Add a detail you have checked, save it, then return here to prepare the preview.</p></>}
   {action.kind==='manual' && <><h3>{redirect?'Choose where visitors should go':'What needs to change'}</h3><p>{diagnosis.external?'Replace the external address in your own content, remove its link while retaining the text, or mark the finding as expected. Shopify redirects cannot repair another website.':action.detail}</p>
    {redirect ? <form id="arena-redirect" onSubmit={e=>{e.preventDefault();setRedirectRequested(true);onAction(mode==='expected'?'markExpectedLink':mode==='redirect'?'applyRedirect':mode==='replace'?'replaceBrokenLink':issue.code==='broken-link'?'removeBrokenLink':'removeReportedUrl',{path,target,resourceId:issue.resourceId});}}>
     <label>Choose a fix<select value={mode} disabled={busy} onChange={e=>setMode(e.target.value)}>{existing&&<option value="restore">Restore the existing page in Shopify (recommended)</option>}{!diagnosis.external&&<option value="redirect">Redirect this retired address</option>}{issue.code==='broken-link'&&<option value="replace">Replace the link on this page</option>}{["broken-link","reported-404"].includes(issue.code) && <option value="delete">{issue.code==="broken-link"?"Delete this link (keep the text)":"Delete from the tracking list"}</option>}{diagnosis.external&&<option value="expected">Mark as expected (keep this link)</option>}</select></label>
   ...
   {(redirectRequested || requested) && feedback && <p role={feedback.ok?'status':'alert'}>{feedback.message}</p>}
  </div>
  <div className="review-footer">
   ...
   {issue.feature==='seo' && action.kind==='generate' && <button type="button" className="button" disabled={busy} onClick={()=>{setRequested(true);onAction('sourceDraft',{id:issue.resourceId});}}>Use source draft now</button>}
   {resource && ['product','collection','article'].includes(resource.kind) && <button type="button" className="button" onClick={onFacts}>Edit source suggestions</button>}
