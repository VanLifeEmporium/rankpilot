import {metadataQuality,legacyWordCap} from './metadata-quality';
import {credentials} from './security.server';
import {htmlValue} from './html-value';
import {semanticLinks} from './semantic-links.server';
import {resourcePath} from './link-diagnosis';
import {checkIndexation} from './indexation.server';
import {sourceDraft} from './source-drafts';
import {recordScore,recordImpact} from './measurement.server';
import {brokenAddress,validRedirectPath} from './broken-url';
import {load as loadHtml} from 'cheerio';
import {removeBrokenLink} from './remove-link';
import {z} from "zod";
...
import { crawlStore, publicFetch, limitedText } from "./crawl.server";
...
export async function enqueueGeneration(storeId:string, ids:string[], feature:Feature, regenerate=false) {
 ...
 const fingerprint=createHash('sha256').update(JSON.stringify({version:14,feature,titleBrandMode:settings(store.settings).titleBrandMode,titleBrand:settings(store.settings).titleBrand,policy:feature==='faq'?policy:undefined,resources:resources.map(r=>[r.id,r.payload,r.facts])})).digest('hex');
...
export function matchesField(actual:unknown, expected:unknown, feature:string):boolean {
 if(['description','links'].includes(feature) && typeof actual==='string' && typeof expected==='string')return htmlValue(actual)===htmlValue(expected);
...
export async function audit(storeId: string) {
  ...
  if (!store.demo) {
    const crawl = await crawlStore(
      store.domain,
      resources,
      Math.max(settings(store.settings).crawlLimit, Math.min(resources.length+1,2000)),
      settings(store.settings).reportedPaths || [],
    );
    ...
    coverage = {
      ...
      crawlLimit: Math.max(settings(store.settings).crawlLimit,Math.min(resources.length+1,2000)),
      externalLinks: "Checked alongside internal links, up to 3,000 unique destinations",
      linkChecks: crawl.discoveries.linkChecks,
      reported: crawl.discoveries.reported,
  ...
  const row = await prisma.audit.create({
    data: {
      ...
      issues: JSON.stringify(result.issues.filter(i=>!(JSON.parse(store.discoveries||"{}").expectedLinks||[]).some((x:{resourceId:string;address:string})=>i.code==="broken-link"&&i.resourceId===x.resourceId&&i.link?.url===x.address))),
...
export async function propose(
  ...
  const pending = await prisma.change.findFirst({
    where: {
      ...
      resourceId,
      feature,
      status: { in: ["pending", "approved", "applying", "verifying"] },
    },
  });
  if (pending) {
    const reasons:string[]=JSON.parse(pending.reasons);
    const outdated = legacyWordCap(reasons) || ((feature === "seo" || feature === "title") && ((t:string)=>!t || t.length>255)(String(feature === "seo" ? JSON.parse(pending.after).title : JSON.parse(pending.after)).trim())) || (["seo","description"].includes(feature) && !reasons.some(trustedSourceReview)) || (feature === "alt" && !reasons.some(r=>r.startsWith("image-reviewed-v1:")));
    if(pending.status !== "pending" || (!linkTarget && !outdated && sameField(JSON.parse(pending.before),fieldValue(p,feature)))) return pending;
    // Keep this draft visible until a reviewed replacement is safely stored.
  }
  if(feature==='seo'&&!sourceOnly){const rejected=await prisma.change.findMany({where:{storeId,resourceId,feature,status:'rejected'},orderBy:{updatedAt:'desc'}});const blocked=feedback.map(e=>({...JSON.parse(e.detail),date:e.createdAt})).find(e=>/current content is better/i.test(e.reason||'')&&rejected.some(r=>r.id===e.changeId&&sameField(JSON.parse(r.before),p.seo)));if(blocked){if(outcome)outcome.message=`You chose to keep the current content on ${new Date(blocked.date).toLocaleDateString('en-GB')}. Automatic replacements are suppressed while this content is unchanged; you can still edit a source draft.`;return null;}}
  const relatedResources=feature==='links'?await prisma.resource.findMany({where:{storeId,id:{not:resource.id},kind:{in:linkTarget?['article','product','collection']:['product','collection']}}}):[];
  const search=feature==='links'?await prisma.metric.findFirst({where:{storeId,provider:'gsc'},orderBy:{period:'desc'}}):null;
  const related=feature==='links'?await semanticLinks(storeId,resource,linkTarget?relatedResources.filter(r=>r.id===linkTarget):relatedResources,search?JSON.parse(search.payload).rows:undefined,!!linkTarget):[];
  if(feature==='links' && !related.length)throw new Error('No sufficiently relevant new links found. Existing content was kept; no filler links were added.');
  const proposal = sourceOnly && feature==="seo" ? sourceDraft(p,resource.kind,cfg) : feature === "title" && (cleanTitle(p.title).length>65)
...
export const trustedSourceReview=(reason:string)=>(reason.startsWith('source-extract-v1:') || reason.startsWith('merchant-reviewed-v1:') || reason.startsWith('source-reviewed-v1:')) && !reason.startsWith('source-reviewed-v1: The existing Shopify page title');
export function assertSafeCopy(feature: string, before: unknown, after: unknown, reasons: string[] = []) {
  if(legacyWordCap(reasons))throw new Error("This older preview used a retired word limit. Generate a new content review before accepting.");
  ...
  if (["title","seo"].includes(feature)) { const title=String(feature === "seo" ? afterMeta?.title || "" : after || "").trim(); if(!title || title.length>255) throw new Error("Regenerate this proposal: the title must be present and under 256 characters."); }
...
export async function approve(storeId: string, id: string, actor: string, expectedVersion?:string) {
  ...
  await prisma.$transaction(async (tx) => {
    const other=await tx.change.findFirst({where:{storeId,resourceId:row.resourceId,id:{not:id},status:{in:['approved','applying','verifying','rolling_back']}}});
    ...
    const claimed = await tx.change.updateMany({
      where: { id, storeId, status: "pending", updatedAt:row.updatedAt },
    ...
    await tx.job.create({
      data: {
        storeId,
        kind: "apply",
        payload: JSON.stringify({ changeId: id, verificationPass:13 }),
...
export async function applyChange(
  ...
  if(!rollback)await recordImpact(storeId,id);
  ...
  if (!matchesField(actual,after,change.feature) && client) {
    ...
    if(!rollback)await prisma.change.update({where:{id},data:{status:'verifying'}});
  }
  ...
  if(!matchesField(fieldValue(next,change.feature),after,change.feature)) {
    await prisma.change.update({where:{id},data:{status:rollback?'verification_failed':'verifying',error:null}});
    throw new Error(rollback?'Verification failed: the restored value is not confirmed yet.':'Confirmation pending: checking the saved update again.');
  }
  ...
  if(change.feature==='links') {
   const marker=JSON.parse(change.reasons).find((s:string)=>s.startsWith('Link fix: '));
   if(marker){const fix=JSON.parse(marker.slice(10));await enqueue(storeId,rollback?'audit':'recheck-link',rollback?{}:{address:fix.address,resourceId:r.id},`link-verification:${id}:${rollback}:${Date.now()}`);}
  }
...
// Reconcile older acknowledged writes once per release without asking the
// merchant to resave them. This only schedules read-only checks.
export async function queueSavedChecks(storeId:string) {
 ...
 for(const row of rows){
  ...
  if(payload.verificationPass===13)continue;
  await prisma.$transaction(async tx=>{
   const claimed=await tx.change.updateMany({where:{id:row.id,storeId,status:'verification_failed'},data:{status:'verifying',error:null}});
   if(!claimed.count)return;
   await tx.job.update({where:{id:job.id},data:{status:'queued',attempts:0,runAt:new Date(),lockedAt:null,error:null,payload:JSON.stringify({...payload,verificationPass:13,verifyOnly:true})}});
  });
 }
}
export async function verifyChange(storeId:string,id:string) {
 ...
 if(!change.approvedBy || !['applied','approved','applying','verifying','apply_failed','verification_failed','conflict'].includes(change.status))throw new Error('Only accepted changes can be verified.');
 ...
 const expected=JSON.parse(change.after);
 const actual=fieldValue(live,change.feature);
 if(!matchesField(actual,expected,change.feature)) {
  const comparison=change.feature==='seo' ? (['title','description'] as const).map(field=>({field,accepted:String(expected?.[field] ?? ''),live:live.seo[field],matches:matchesField({title:live.seo[field],description:''},{title:expected?.[field],description:''},'seo')})) : [{field:change.feature,accepted:typeof expected==='string'?expected:JSON.stringify(expected),live:typeof actual==='string'?actual:JSON.stringify(actual),matches:false}];
  await prisma.resource.update({where:{id:resource.id},data:{payload:JSON.stringify(live),title:live.title,handle:live.handle}});
  await log(storeId,'Live verification mismatch',{changeId:id,resourceId:resource.remoteId,kind:resource.kind,feature:change.feature,fields:comparison?.filter(f=>!f.matches).map(f=>f.field)});
  return {ok:false,message:'The saved content does not match this preview. This check made no further edits; the earlier save may have changed the page. Review the current Shopify content before preparing another change.',comparison};
 }
 await prisma.$transaction([
...
async function scheduleIndexRecheck(storeId:string,resource:{id:string;title:string;kind:string;handle:string;payload:string}) {
 try {const store=await prisma.store.findUniqueOrThrow({where:{id:storeId}});if(store.demo)return;const cfg=settings(store.settings);if(!cfg.gscSite)return;const keys=await credentials(storeId);if(!keys.googleServiceAccount&&!keys.googleRefreshToken)return;await enqueue(storeId,'indexation',{url:new URL(resourcePath(resource),store.domain).href},`after-apply:${resource.id}:${Date.now()}`);}catch(error){await log(storeId,'Saved update; index recheck could not be queued',{resourceId:resource.id,message:(error as Error).message});}
}
...
export async function executeJob(job: {
  ...
  switch (job.kind) {
    ...
    case "apply": {
      const change=await prisma.change.findFirstOrThrow({where:{id:p.changeId,storeId:job.storeId}});
      if(p.verifyOnly || ['verifying','verification_failed'].includes(change.status)) {
        // The mutation was acknowledged. Every subsequent automatic attempt is
        // read-only, including recovery after a worker restart.
        const result=await verifyChange(job.storeId,p.changeId);
        if(!result.ok)throw new Error('Confirmation pending: checking the saved update again.');
        return;
      }
    ...
    case "analytics":
      ...
      await recordScore(job.storeId);
      const store=await prisma.store.findUniqueOrThrow({where:{id:job.storeId}});
      if(!store.demo && settings(store.settings).gscSite){
        const keys=await credentials(job.storeId);
        const previous=await prisma.metric.findFirst({where:{storeId:job.storeId,provider:'indexation'}});
        if((keys.googleServiceAccount||keys.googleRefreshToken) && !previous)await enqueue(job.storeId,'indexation',{},'first-connected-check');
      }
    ...
    case "visibility":
      try {return await trackVisibility(job.storeId);}finally {await recordScore(job.storeId);}
    case "recheck-link":
      return recheckLink(job.storeId,p.address,p.resourceId);
    case "indexation":
      {const result=await checkIndexation(job.storeId,{url:p.url,onProgress:async(checked,total)=>{await prisma.job.update({where:{id:job.id},data:{payload:JSON.stringify({...p,progress:{checked,total}})}});}});await recordScore(job.storeId);return result;}
    case "pagespeed":
      {const result=await pageSpeed(job.storeId, p.url);await recordScore(job.storeId);return result;}
    case "refresh-audit":
...
export async function tick(options:{lane?:'apply'|'background'|'generation';jobId?:string}={}) {
  ...
  const filter={status:'queued',runAt:{lte:new Date()},attempts:{lt:5},...(options.jobId?{id:options.jobId}:{}),...(options.lane?{kind:options.lane==='apply'?{in:['apply','rollback']}:options.lane==='generation'?{in:['optimise']}:{notIn:['apply','rollback','optimise']}}:{})};
  const priority=options.lane && options.lane!=='apply'?null:await prisma.job.findFirst({where:{...filter,kind:{in:['apply','rollback']}},orderBy:{createdAt:'asc'}});
  ...
  try {
    ...
    const terminal =
      next.attempts >= 3 || (!/^Confirmation pending:/.test(error) && (['draft','visibility'].includes(next.kind) || /Verification failed|Needs manual review/.test(error) ||
    ...
    if(['apply','rollback'].includes(next.kind)) {
      ...
      if(next.kind==='apply' && terminal)await prisma.change.updateMany({where:{id:changeId,storeId:next.storeId,status:'verifying'},data:{status:'verification_failed',error:'We could not confirm this update. Check the saved result below; checking will not change your shop.'}});
    ...
    if(terminal)await log(next.storeId, "Update needs attention", { kind: next.kind, error });
  } finally {
...
export async function proposeRedirect(
  ...
  if(!validRedirectPath(path)||!validRedirectPath(target,true)||path===target)throw new Error('Choose distinct store paths, excluding cart, checkout and account actions.');
  const client = await clientFor(storeId);
  if(client){
    const store=await prisma.store.findUniqueOrThrow({where:{id:storeId}});
    const response=await publicFetch(new URL(path,store.domain).href,{method:'GET'});await response.body?.cancel();
    if(![404,410].includes(response.status)){if(response.ok)await recheckLink(storeId,new URL(path,store.domain).href);throw new Error(`The audit recorded a missing page earlier, but a fresh check now returns HTTP ${response.status}. No redirect was created. Its current status has been rechecked; working destinations are cleared automatically.`);}
  }
  if (!client) {
    const records = await prisma.resource.findMany({where:{storeId,kind:'collection'}});
    if(records.some(r=>'/collections/'+r.handle===path)) throw new Error('The old collection is still present. Only missing pages can be redirected.');
    if(target!=='/' && !records.some(r=>'/collections/'+r.handle===target)) throw new Error('Choose a known collection destination in demo mode.');
  }
  ...
  return prisma.change.create({
    data: {
      ...
      reasons: JSON.stringify([
        "Repair the page redirect to the selected verified live destination. Rollback restores the previous mapping, or removes a newly created mapping, only if nobody has edited it.",
      ]),
    },
  });
}
...
async function applyRedirect(storeId: string, change: Change, rollback: boolean) {
  ...
  await enqueue(storeId,rollback?'audit':'recheck-link',rollback?{}:{address:mapping.path},`redirect-verification:${change.id}:${rollback}:${Date.now()}`);
  await log(
    storeId,
...
/** Explicit merchant request: remove only matching anchors, retaining their contents. */
export async function proposeLinkRemoval(storeId:string,resourceId:string,address:string,target?:string){
 const r=await prisma.resource.findFirstOrThrow({where:{id:resourceId,storeId,kind:{in:['product','collection','article','page']}}});
 const store=await prisma.store.findUniqueOrThrow({where:{id:storeId}});
 const client=await clientFor(storeId);
 const p:Payload=client?await fetchResource(client,r.remoteId,r.kind):JSON.parse(r.payload);
 const base=new URL(p.url || `/${r.kind==='article'?`blogs/${p.blogHandle}`:r.kind+'s'}/${r.handle}`,store.domain).href;
 if(target){
  if(/^https:\/\//i.test(target)){const u=new URL(target);if(u.username||u.password)throw new Error('Use a public HTTPS destination without embedded credentials.');const response=await publicFetch(u.href);await response.body?.cancel();if(!response.ok)throw new Error(`The replacement returned HTTP ${response.status}; choose a working URL.`);}
  else {if(!validRedirectPath(target,true))throw new Error("Choose a valid store path or a complete HTTPS URL.");if(client)await verifyRedirectTarget(storeId,target);}
 }
 const removed=removeBrokenLink(p.descriptionHtml,address,base,target);
 return prisma.change.create({data:{storeId,resourceId,feature:'links',before:JSON.stringify(p.descriptionHtml),after:JSON.stringify(removed.html),reasons:JSON.stringify([`Merchant requested ${target?"replacement":"removal"} of ${removed.removed} link(s) to ${address}${target?" with "+target:""}. Visible text is retained. Undo restores the previous content if it has not subsequently changed.`,"Link fix: "+JSON.stringify({address})]),blockers:'[]'}});
}

/** Recheck the shared destination, or the exact edited source, before clearing findings. */
export async function recheckLink(storeId:string,address:string,resourceId?:string) {
 const store=await prisma.store.findUniqueOrThrow({where:{id:storeId}});
 const resources=await prisma.resource.findMany({where:{storeId}});
 const absolute=new URL(address,store.domain).href;
 let resolved=false,status:number|null=null;
 if(resourceId){
  const r=resources.find(r=>r.id===resourceId);if(!r)throw new Error('Source page no longer exists. Run a new scan.');
  const p:Payload=JSON.parse(r.payload);
  const url=new URL(p.url||`/${r.kind==='article'?`blogs/${p.blogHandle}`:r.kind+'s'}/${r.handle}`,store.domain).href;
  let html=p.descriptionHtml;
  if(!store.demo){const response=await publicFetch(url);status=response.status;if(!response.ok)return [{message:`The edited page returned HTTP ${status}. Its link finding is retained until a successful check.`}];html=await limitedText(response);}
  const $=loadHtml(html);resolved=!$('a[href]').toArray().some(el=>{try{return new URL($(el).attr('href')!,url).href===absolute;}catch{return false;}});
 }else if(store.demo){resolved=true;status=200;}else {const response=await publicFetch(absolute);status=response.status;await response.body?.cancel();resolved=response.ok;}
 let removed=0,pages=0;
 await prisma.$transaction(async tx=>{
  if(!resourceId){
   const current=await tx.store.findUniqueOrThrow({where:{id:storeId}});
   const discoveries=JSON.parse(current.discoveries||'{}');
   if(Array.isArray(discoveries.reported)){
    discoveries.reported=discoveries.reported.map((r:{path:string;status?:number;state?:string})=>{
     try{if(new URL(r.path,store.domain).href===absolute)return {...r,status,checkedAt:new Date().toISOString(),state:resolved?'Working':status===404||status===410?'Missing page':'Check unavailable'};}catch{/* retain malformed historical evidence */}
     return r;
    });
    await tx.store.update({where:{id:storeId},data:{discoveries:JSON.stringify(discoveries)}});
   }
  }
  const audit=await tx.audit.findFirst({where:{storeId},orderBy:{createdAt:'desc'}});if(!audit)return;
  const issues:import('./types').Issue[]=JSON.parse(audit.issues);
  const matches=(i:import('./types').Issue)=>['broken-link','404','reported-404'].includes(i.code)&&(!resourceId||i.resourceId===resourceId)&&(()=>{try{return new URL(brokenAddress(i,store.domain,resources.find(r=>r.id===i.resourceId)),store.domain).href===absolute;}catch{return false;}})();
  const found=issues.filter(matches);pages=new Set(found.map(i=>i.resourceId)).size;
  removed=resolved?found.length:0;
  const next=resolved?issues.filter(i=>!matches(i)):issues;
  await tx.audit.update({where:{id:audit.id},data:{issues:JSON.stringify(next),coverage:JSON.stringify({...JSON.parse(audit.coverage),lastLinkRecheck:{address,status,resolved,removed,checkedAt:new Date().toISOString()}})}});
 });
 const message=resolved?`Checked ${address}: ${removed} findings cleared across ${pages} pages. ${resourceId?'The broken link is no longer present.':'The destination now responds successfully.'}`:`Checked ${address}: ${status?`HTTP ${status}`:'the link is still present'}. Findings remain open.`;
 await log(storeId,'Broken-link check completed',{address,status,resolved,removed,pages});
 return [{message}];
}
