...
import {canonicalPage,queryTotals} from './analytics';
...
export async function pageSuggestions(storeId:string,id:string){
 ...
 const pagePath=(url:string)=>new URL(canonicalPage(url),'https://store.invalid').pathname.replace(/\/$/,'');
 const queryEvidence=queryTotals(rows).filter(r=>pagePath(r.keys[0]||'')===pagePath(resourcePath(resource))&&r.keys[1]).sort((a,b)=>b.clicks-a.clicks||b.impressions-a.impressions);
 const candidateQueries=queryEvidence.map(r=>r.keys[1]);
 const queries=[...new Set<string>(candidateQueries)];
 suggestions.primary=queries[0]||'';suggestions.secondary=queries.slice(1,6);
 const top=queryEvidence[0];
 const keywordSource=top?`Search Console: ${top.clicks} clicks / ${top.impressions} impressions${top.impressions>=10&&top.position!==undefined?` / average position ${top.position.toFixed(1)}`:' / too few impressions to report a position'}. Ordered by clicks, then impressions.`:'No Search Console query is available for this page. Leave blank or enter a phrase you have checked; no search demand is assumed.';
...
export async function collectionCopy(storeId:string,id:string){
 ...
 const items=plan.collection.children;const phrase=plan.savedKeyword||plan.suggestions.primary||p.title;
