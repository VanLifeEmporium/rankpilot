...
export async function crawlStore(
  ...
  await mapConcurrent(targets,4,async target => {
    try {
      ...
      discoveries.schemas.push({ url: target.url, types, sources:schema.sources, errors:[...schema.errors,...validateSchema(schema.nodes)],checkedAt:new Date().toISOString(),valid: !schema.errors.length && !validateSchema(schema.nodes).length });
