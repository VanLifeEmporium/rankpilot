...
import {validRedirectPath} from './broken-url';
...
export async function proposeRedirect(
  ...
  if(client){
    ...
    if(![404,410].includes(response.status))throw new Error('The old address is not a confirmed missing page. Inspect it before redirecting.');
  }
  if (!client) {
...
export async function proposeLinkRemoval(storeId:string,resourceId:string,address:string){
 ...
 const removed=removeBrokenLink(p.descriptionHtml,address,base);
 return prisma.change.create({data:{storeId,resourceId,feature:'links',before:JSON.stringify(p.descriptionHtml),after:JSON.stringify(removed.html),reasons:JSON.stringify([`Merchant requested removal of ${removed.removed} link(s) to ${address}. Visible text is retained. Undo restores the previous content if it has not subsequently changed.`]),blockers:'[]'}});
}
