...
export function removeBrokenLink(html:string, address:string, base:string){
 ...
 $('a[href]').each((_,el)=>{try{if(new URL($(el).attr('href')!,base).href===target.href){$(el).replaceWith($(el).contents());removed++;}}catch{/* Leave unrelated links unchanged. */}});
