...
export default function Workspace() {
  ...
  const severity=searchParams.get('priority') || 'all';
  const issueQuery=searchParams.get('q') || '';
  const issueCode=searchParams.get('finding') || 'all';
  function setFilter(key:string,value:string){setSearchParams(previous=>{const next=new URLSearchParams(previous);if(value && value!=='all')next.set(key,value);else next.delete(key);return next;},{replace:true,preventScrollReset:true});}
  const setSeverity=(v:string)=>setFilter('priority',v);
  const setIssueQuery=(v:string)=>setFilter('q',v);
  const setIssueCode=(v:string)=>setFilter('finding',v);
  ...
  useEffect(()=>{
    // Keep a light status poll alive even if a fast job finishes between loaders.
    // Stable timer: fetcher transitions must not continually reset the interval.
    const check=()=>{const current=progressRef.current;if(current.state==='idle' && document.visibilityState==='visible')current.load('/app/job-status');};
    const timer=setInterval(check,applying?750:running?1500:5000);
  ...
  useEffect(()=>{
    if(progress.data?.revision && progress.data.revision!==loadedRevision && revalidator.state==='idle')void revalidator.revalidate();
    ...
    <div className="workspace">
      ...
      <div className="main-wrap">
        ...
        <main>
          ...
          {blockedPending.length>0 && <p className="notice">{pending.length} ready to accept · {blockedPending.length} need a new review. <Link to="/app/reviews">Open review queue</Link>. Generating a replacement keeps the old draft until the new one is ready.</p>}
            ...
            <>
              <div className="metrics-grid">
                ...
                <Metric
                  ...
                  caption="Core product attributes with a value, source and confirmation"
                />
                <Metric
        ...
        {review && (
          <>
            ...
            <div className="preview-meta">
              ...
              {["title","seo"].includes(review.feature) && <span>{review.feature === "seo" ? "Search title only; the visible page title stays unchanged." : "Updates the visible page title."} Maximum five words and 60 characters, including any brand suffix.</span>}
            </div>
            ...
            {review.approvedBy && !['draft','redirect'].includes(review.feature) && ['applied','approved','applying','apply_failed','verification_failed','conflict'].includes(review.status) && <Button disabled={busy} onClick={()=>submit('verify',{id:review.id})}>Verify in Shopify</Button>}
