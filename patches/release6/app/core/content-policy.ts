...
export function titleBrandInstructions(cfg:Settings){
 ...
 const rule=cfg.titleBrandMode==='omit'?'Omit the store-name suffix only. Preserve product manufacturer and model identity.':cfg.titleBrandMode==='append' && brand?`End the search title with the store brand “${brand}”. Fit the whole title within both limits; if accurate page identity cannot fit, return changed=false and explain.`:'Preserve an existing store-brand suffix where it fits both limits; do not remove a product manufacturer or model just to shorten the title. If the identity cannot fit, return changed=false.';
 return 'Search titles must satisfy BOTH limits: at most five words AND 60 characters including the brand suffix. '+rule;
}
